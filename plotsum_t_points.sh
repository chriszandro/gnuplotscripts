...
#  set term png medium font arial 8
  set term pngcairo font "Arial, 7"
  ...
  set multiplot layout 2,2 rowsfirst title "${FILE}" 
    ...
    plot "${FILE}" using 1:3 with points ls 2

    # --- GRAPH Position
    set title "Position"
    set xlabel "time [a.u.]"
    set ylabel "Position [Bohr Radius]"
    plot "${FILE}" using 1:2 with points ls 3

    # --- GRAPH 3
    set title "Energy"
    set xlabel "time [a.u.]"
    set ylabel "Energy [eV]"
    plot "${FILE}" using 1:4 with points ls 4

    # --- GRAPH d
    set title "Occupation"
    set xlabel "time [a.u.]"
    set ylabel "n"
    plot "${FILE}" using 1:5 with points ls 1, "${FILE}" using 1:6 with points ls 5
...
#    set term pdf font "Arial,4"
    set term pdfcairo font "Arial,7"
